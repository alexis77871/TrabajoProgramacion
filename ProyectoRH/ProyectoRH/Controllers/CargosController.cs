...
namespace ProyectoRH.Controllers
{
    public class CargosController : Controller
    {
        ...
        {
            var hola = (from k in db.Cargos
                        select k);
            if (!String.IsNullOrEmpty(searchString))
            {
                hola = hola.Where(k => k.Cargo.Contains(searchString));
            }
        ...
        public ActionResult Create([Bind(Include = "CodigoCargo,Cargo")] Cargos cargos)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "CodigoCargo,Cargo")] Cargos cargos)
        {
            if (ModelState.IsValid)
            {
